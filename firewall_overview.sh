#!/usr/bin/env bash
# firewall_overview.sh  –  Auto-detects iptables/nftables and shows firewall overview
#
# Auto-detection logic:
#   1. iptables-save  →  if ≥3 -A rule lines present, use iptables engine (full Bash)
#      (covers iptables-legacy AND iptables-nft; gives full multiport/ctstate detail)
#   2. nft             →  fallback for pure nftables: delegates to python3 firewall_overview.py
#
# Requirements: bash 4+, sudo, iptables-save, grep -P
# For pure nftables: python3 + firewall_overview.py (in same directory)
#
# Usage:
#   bash firewall_overview.sh [--backend auto|iptables|nft] [-o FILE]
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --backend)
            case "$2" in
                auto|iptables|nft) BACKEND_ARG="$2"; shift 2 ;;
                *) echo "Unknown backend: $2  (use: auto, iptables, nft)" >&2; exit 1 ;;
            esac ;;
        -h|--help)
            echo "Usage: $0 [--backend auto|iptables|nft] [-o FILE]"
            echo "  --backend    Force a specific backend (default: auto)"
            echo "  -o, --output Write plain-text output to FILE"
            echo "               (default: firewall-overview-<hostname>.txt)"
            exit 0 ;;
...
[[ -z "$OUTFILE" ]] && OUTFILE="firewall-overview-${FQDN}.txt"
...
detect_backend() {
    ...
    if command -v iptables-save &>/dev/null; then
        local raw; raw=$(sudo iptables-save 2>/dev/null) || true
        local cnt; cnt=$(echo "$raw" | grep -c '^-A ' 2>/dev/null || echo 0)
        if [[ $cnt -ge 3 ]]; then
            local variant="iptables"
            if iptables --version 2>/dev/null | grep -q 'nf_tables'; then
                ...
                variant="iptables-legacy"
            fi
            BACKEND="iptables"
            BACKEND_LABEL="iptables / ${variant} – auto-detected"
            IPTS="$raw"
            return
        fi
    fi
    ...
    if command -v nft &>/dev/null; then
        local cnt; cnt=$(sudo nft -j list ruleset 2>/dev/null | grep -c '"rule"' || echo 0)
        if [[ $cnt -ge 1 ]]; then
            BACKEND="nft"
            BACKEND_LABEL="nftables – auto-detected"
            return
        fi
    fi
    ...
    if [[ -n "$IPTS" ]]; then
        BACKEND="iptables"
        BACKEND_LABEL="iptables – auto-detected (few rules)"
        return
    fi

    echo "Error: No firewall backend available (iptables-save and nft both failed)." >&2
    exit 1
}
...
case "$BACKEND_ARG" in
    ...
    iptables)
        if ! command -v iptables-save &>/dev/null; then
            echo "Error: iptables-save not found." >&2; exit 1
        fi
        IPTS=$(sudo iptables-save 2>/dev/null) || { echo "Error: iptables-save failed." >&2; exit 1; }
        BACKEND="iptables"
        BACKEND_LABEL="iptables – forced"
        ;;
    nft)
        BACKEND="nft"
        BACKEND_LABEL="nftables – forced"
        ;;
esac

# ── nft: delegate to python3 ─────────────────────────────────────────────────
# Full nft JSON parsing in Bash would require jq and is not maintainable.
# Delegate to firewall_overview.py --backend nft if python3 is available.
if [[ "$BACKEND" == "nft" ]]; then
    PY=""
    if command -v python3 &>/dev/null; then
        if [[ -f "$SCRIPT_DIR/firewall_overview.py" ]]; then
            PY="python3 $SCRIPT_DIR/firewall_overview.py --backend nft"
        elif [[ -f "$SCRIPT_DIR/nft_overview.py" ]]; then
            PY="python3 $SCRIPT_DIR/nft_overview.py"
        fi
    fi

    if [[ -n "$PY" ]]; then
        exec $PY ${OUTFILE:+-o "$OUTFILE"}
    else
        cat >&2 <<'EOF'
Error: Pure nftables system detected.

The Bash script delegates nft JSON parsing to python3 firewall_overview.py.
Neither was found. Options:
  1. Install python3 and place firewall_overview.py in the same directory.
  2. Run: sudo nft -j list ruleset | python3 firewall_overview.py --backend nft
EOF
        exit 1
    fi
fi
...
# ═════════════════════════════════════════════════════════════════════════════
# iptables ENGINE  (BACKEND == "iptables")
# ═════════════════════════════════════════════════════════════════════════════

# Tee: stdout → color, file → plain text
...
join_csv() {
    local sep="" result=""
    for p in "$@"; do result="${result}${sep}${p}"; sep=", "; done
    echo "$result"
}
...
print_summary() {
    ...
    local allow_str=""
    [[ ${#allow_parts[@]} -gt 0 ]] && allow_str=$(join_csv "${allow_parts[@]}")
    printf "  ${GRN}+ INGRESS ALLOW:${R}  %s,  RELATED/ESTABLISHED\n" "${allow_str:-(none)}"

    if [[ ${#drop_parts[@]} -gt 0 ]]; then
        local drop_str; drop_str=$(join_csv "${drop_parts[@]}")
    ...
    if [[ ${#DNAT_PORTS[@]} -gt 0 ]]; then
        ...
        local dnat_str; dnat_str=$(join_csv "${dnat_parts[@]}")
        printf "  ${MGT}> NAT/DNAT:    ${R}  %s\n" "$dnat_str"
        printf "  ${GRY}               (DNAT ports bypass INPUT filter – traffic routed via FORWARD to container)${R}\n"
    fi

    local e_allow_str=""
    [[ ${#e_allow_parts[@]} -gt 0 ]] && e_allow_str=$(join_csv "${e_allow_parts[@]}")
    if [[ -n "$e_allow_str" ]]; then
        printf "  ${YEL}+ EGRESS ALLOW: ${R}  %s,  RELATED/ESTABLISHED,  DEFAULT ALLOW\n" "$e_allow_str"
    else
        printf "  ${YEL}+ EGRESS ALLOW: ${R}  RELATED/ESTABLISHED,  DEFAULT ALLOW\n"
    fi

    if [[ ${#e_drop_parts[@]} -gt 0 ]]; then
        local e_drop_str; e_drop_str=$(join_csv "${e_drop_parts[@]}")
...
echo
printf "${B}%s${R}\n" "$SEP_D"
printf "${B}  Firewall Overview  –  %s  [%s]${R}\n" "$FQDN" "$BACKEND_LABEL"
printf "${B}%s${R}\n" "$SEP_D"
